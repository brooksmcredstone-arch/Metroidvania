@tool
@icon("res://metroidvania_project/general/icons/map_node.svg")
...
@onready var transistion_block : Control = %TransistionBlock
...
func _ready() -> void:
	if Engine.is_editor_hint():
		...
		if not SaveManager._is_area_discovered(linked_scene):
			visible = false
...
func update_node() -> void:
	...
	#loads and checks level validity
		...
		if packed_scene:
			...
			if instance:
				...
				for c in instance.get_children():
					if c  is LevelBounds:
						#checking for the size of the area's level boundary and position
						new_size = Vector2(c.width, c.height)
						indicator_offset = c.position
					elif c is LevelTransition:
						#checks for any level transitions that exist in the level it is holding
						transitions.append(c)
				instance.queue_free()
	
	#Change the size to appropiately fit the map
...
func update_node_label(scene : Node) -> void:
	...
	t = t.replace("res://metroidvania_project/level/","")
	t  = t.replace(".tscn", "")
	label.text = t
	pass

func create_entrance_data( transitions : Array) -> void:
...
func create_transition_blocks() -> void:
	if not transistion_block:
		transistion_block = %TransistionBlock
	
	for c in transistion_block.get_children():
		c.queue_free()
...
func add_block() -> ColorRect:
	...
	transistion_block.add_child(block)
...
func display_player_location() -> void:
	...
	i.position = pos
	var posclamp : Vector2 = Vector2(3,3)
	pos = pos.clamp(position + posclamp, position + size - posclamp)
	pass
